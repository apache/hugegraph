...
| 边界 | 执行证据 | 结果 | 仍未证明 |
| --- | --- | --- | --- |
| PD Partition | `pd-core-test,pd-common-test`；`*Partition*Test`；日志 `/tmp/pd-partition-all.log`；PD0 故障证据 `docs/evidence/rust-rewrite/compose-pd0-fault-recovery-20260913.log` | Partition 32/32；PD Core/Common 187 项执行、2 项跳过、0 失败；pd0 停止期间 pd1/pd2 健康，恢复后 3 节点健康 | 网络分区、差分回放、完整成员变更矩阵 |
