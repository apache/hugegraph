...
状态：审核版模板；“已发现”不等于“已通过”。本矩阵是迁移的强制门禁，实际通过状态须由 POC 和测试报告填写。
...
| 边界 | 执行证据 | 结果 | 仍未证明 |
| --- | --- | --- | --- |
| PD Partition | `pd-core-test,pd-common-test`；`*Partition*Test`；日志 `/tmp/pd-partition-all.log`；PD0 故障证据 `docs/evidence/rust-rewrite/compose-pd0-fault-recovery-20260913.log`、网络分区证据 `docs/evidence/rust-rewrite/compose-pd0-network-partition-20260913.log`；独立历史 Oracle `tools/raft-linearizability/` 及 `docs/evidence/rust-rewrite/raft-linearizability-oracle-20260913-concurrent.log`、`docs/evidence/rust-rewrite/raft-linearizability-oracle-20260913-cli.log` | Partition 32/32；PD Core/Common 187 项执行、2 项跳过、0 失败；pd0 停止期间 pd1/pd2 健康，恢复后 3 节点健康；单寄存器顺序/重叠历史正反例和 CLI 错误分类均通过 | 差分回放、完整成员变更矩阵、从真实三节点采集历史并通过线性一致性检查、数据无损检查 |
| Store 基线 | `tools/rust-rewrite-cluster/smoke.sh`；最新成功归档 `docs/evidence/rust-rewrite/e2e-recovery-final4-20260913.log` | 三节点启动、Schema/顶点写入、PD/Store 停止恢复及恢复后读取通过 | 活动快照损坏、完整 Raft 故障矩阵 |
| Store Snapshot | `SnapshotHandlerTest`、`HgBusinessImplTest`；`docs/evidence/rust-rewrite/snapshot-handler-maven-20260913.log`、`docs/evidence/rust-rewrite/hg-business-snapshot-maven-20260913.log` | Java 快照处理器与业务快照测试通过 | Rust 实现 round-trip、损坏快照加载失败语义 |
...
| Rust Partition POC | `tools/rust-partition-poc`；`cargo test`、`cargo test --release`、`cargo-mutants` | 18 项测试通过；契约范围 29 个 mutants 中 27 个捕获、2 个不可变异，mutation score 100%（非契约 `main()` 已排除） | 生产实现的 Java/Rust 差分回放与真实集群验证 |
